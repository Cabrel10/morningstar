...
class MorningstarModel:
    ...
    def __init__(self, model_config: Dict = None, use_llm: bool = True, llm_fallback_strategy: str = 'technical_projection'):
        """
    ...
    def predict(
        ...
        instrument_data: np.ndarray = None) -> Dict[str, np.ndarray]:
        """
        ...
        try:
            ...
            if mcp_data is None:
                ...
                raise ValueError(f"Shape MCP incorrecte: attendu {self.config['mcp_input_dim']}, reçu {mcp_data.shape[1]}")
            ...
            if hmm_data is None:
                ...
                raise ValueError(f"Shape HMM incorrecte: attendu {self.config['hmm_input_dim']}, reçu {hmm_data.shape[1]}")
