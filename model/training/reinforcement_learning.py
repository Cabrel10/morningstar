    ...
    def evaluate(self, n_episodes=10, deterministic=True):
        ...
        for episode in range(n_episodes):
            ...
            if episode_info and 'nav' in episode_info:
                ...
                logger.info(f"Episode {episode+1}/{n_episodes}, Reward: {float(episode_reward):.2f}, NAV: {nav_value:.2f}")
            else:
        ...
        if valid_infos:
            ...
            def safe_mean(key, default=0.0):
                values = [info.get(key, default) for info in valid_infos if isinstance(info.get(key), (int, float, np.number))]  
...
def load_and_prepare_data(data_path, price_cols=None, feature_cols=None, split_ratio=0.8):
    ...
    if feature_cols is None:
        ...
        feature_cols = [col for col in df.columns 
                      if col not in price_cols and col not in ['timestamp', 'date', 'symbol', 'split']]
    ...
    train_prices = train_df[price_cols]
    train_features = train_df[feature_cols]
    
    test_prices = test_df[price_cols]
...
def train_rl_agent(data_path, model_path, hybrid_model_path=None, use_cnn_lstm=True, use_cot=True, 
                  window_size=60, initial_balance=10000.0, reward_weights=None,
                  total_timesteps=100000, evaluation_episodes=10):
    """
