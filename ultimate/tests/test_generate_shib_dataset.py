...
def test_llm_integration(setup_test_dirs):
    ...
    with open(cache_path, "w") as f:
        json.dump({"symbol": test_symbol, "date": test_date, "embedding": test_embedding.tolist()}, f)
...
def test_main_function(setup_test_dirs, monkeypatch):
    ...
    monkeypatch.setattr("scripts.generate_shib_dataset.CACHE_INSTRUMENTS_DIR", setup_test_dirs["instruments_dir"])
    monkeypatch.setattr(
        "scripts.generate_shib_dataset.RAW_PATH", setup_test_dirs["raw_dir"] / "shib_usdt_binance_4h.csv"
    )
    monkeypatch.setattr(
        "scripts.generate_shib_dataset.PROCESSED_PATH",
        setup_test_dirs["processed_dir"] / "shib_usdt_binance_4h_processed.parquet",
    )
    ...
    with patch("subprocess.run") as mock_run, patch("scripts.generate_shib_dataset.load_raw_data") as mock_load, patch(
        "scripts.generate_shib_dataset.clean_data"
    ) as mock_clean, patch("scripts.generate_shib_dataset.create_shib_llm_embeddings") as mock_embeddings, patch(
        "scripts.generate_shib_dataset.apply_feature_pipeline"
    ) as mock_features, patch(
        "scripts.generate_shib_dataset.build_labels"
    ) as mock_labels:
...
def test_abs_error_fix():
    ...
    df = pd.DataFrame({"mcp_0": ["0.01"], "mcp_1": [0.02]})  # Valeur en string plutôt qu'en float
