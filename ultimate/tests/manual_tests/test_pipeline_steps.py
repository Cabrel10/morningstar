...
def run_manual_pipeline_test(input_csv_path: str = "data/raw/eth_raw.csv"):
    ...
    try:
        ...
        assert (
            len(df_cleaned) > min_rows_threshold
        ), f"Expected > {min_rows_threshold} rows after cleaning, found {len(df_cleaned)}"
    ...
    try:
        ...
        assert (
            len(df_features) > min_rows_threshold
        ), f"Expected > {min_rows_threshold} rows after feature engineering, found {len(df_features)}"
    ...
    try:
        ...
        assert (
            len(df_final) > min_rows_threshold
        ), f"Expected > {min_rows_threshold} rows after labeling, found {len(df_final)}"
        print(f"Assertion Passed: Rows > {min_rows_threshold} ({len(df_final)})")

        # Vérification du nombre de colonnes final (cible = 38)
        expected_cols = 38
        assert (
            len(final_columns) == expected_cols
        ), f"Expected exactly {expected_cols} columns, found {len(final_columns)}"
