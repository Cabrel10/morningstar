...
def evaluate_individual(individual, price_data, feature_data, cfg):
    ...
    env = create_trading_env_from_data(
        price_data=price_data, feature_data=feature_data, window_size=cfg.get_config("model.window_size", 60)
    )
...
def optimize_hyperparams(data_path, population_size=20, generations=10, seed=42, output_dir="outputs/ga"):
    """
    ...
    # Liste complète des attributs à inclure dans un individu
    individual_attrs = (
        toolbox.attr_lr,
        toolbox.attr_n_steps,
        toolbox.attr_batch,
        toolbox.attr_epochs,
        toolbox.attr_gamma,
        toolbox.attr_gae,
        toolbox.attr_clip,
        toolbox.attr_pos_size,
        toolbox.attr_sl_pct,
        toolbox.attr_tp_pct,
        toolbox.attr_order_type,
        toolbox.attr_slippage,
        toolbox.attr_cot_weight,
    )
    toolbox.register("individual", tools.initCycle, creator.Individual, individual_attrs, n=1)
    toolbox.register("population", tools.initRepeat, list, toolbox.individual)

    # Register GA operators
    toolbox.register("evaluate", evaluate_individual, price_data=price_data, feature_data=feature_data, cfg=cfg)
    ...
    pop, logbook = algorithms.eaSimple(  # Utiliser algorithms.eaSimple
        pop, toolbox, cxpb=0.5, mutpb=0.2, ngen=generations, stats=stats, halloffame=hof, verbose=True
    )
...
if __name__ == "__main__":
    ...
    best = optimize_hyperparams(
        data_path=args.data_path, population_size=args.pop, generations=args.gen, output_dir=args.out
    )
