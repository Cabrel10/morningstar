...
def build_enhanced_hybrid_model(
    ...
    def create_encoder(input_tensor, num_units, name_prefix):
        ...
        x = Dense(
            num_units, activation="relu", kernel_regularizer=regularizers.l2(0.001), name=f"{name_prefix}_dense1"
        )(input_tensor)
        x = BatchNormalization(name=f"{name_prefix}_bn1")(x)
        x = Dropout(0.3, name=f"{name_prefix}_dropout1")(x)
        x = Dense(
            num_units // 2, activation="relu", kernel_regularizer=regularizers.l2(0.001), name=f"{name_prefix}_dense2"
        )(x)
    ...
    if use_cnn_lstm:
        ...
        if tech_features is None:
            ...
            if tech_input_shape[0] % time_steps != 0:
                raise ValueError(
                    f"La dimension d'entrée technique {tech_input_shape[0]} n'est pas divisible par time_steps {time_steps}"
                )
        ...
        for i, (filters, kernel_size, pool_size) in enumerate(zip(cnn_filters, cnn_kernel_sizes, cnn_pool_sizes)):
            ...
            x = Conv1D(
                filters=filters, kernel_size=kernel_size, activation="relu", padding="same", name=f"cnn_conv_{i+1}"
            )(x)
        ...
        for i, units in enumerate(lstm_units):
            ...
            if bidirectional_lstm:
                ...
                x = LSTM(
                    units, return_sequences=return_sequences, recurrent_dropout=lstm_dropout_rate, name=f"lstm_{i+1}"
                )(x)
    ...
    concatenated_features = Concatenate(name="fusion_concat")(
        [tech_encoded, llm_encoded, mcp_encoded, hmm_encoded, inst_emb_flat]
    )

    # Add Dense layers for richer fusion before shared core
    fused = Dense(256, activation="relu", kernel_regularizer=regularizers.l2(0.001), name="fusion_dense1")(
        concatenated_features
    )
    fused = BatchNormalization(name="fusion_bn1")(fused)
    fused = Dropout(0.4, name="fusion_dropout1")(
        fused
    )  # Keep dropout rate consistent? Or adjust? Let's use 0.4 for now.
    ...
    shared_output = Dense(64, activation="relu", kernel_regularizer=regularizers.l2(0.001), name="shared_dense_output")(
        z
    )
    ...
    if "volatility_quantiles" in active_outputs:
        volatility_quantiles = Dense(num_volatility_quantiles, activation="linear", name="volatility_quantiles")(
            shared_output
        )
    ...
    model = Model(
        inputs=inputs_dict, outputs=outputs, name="enhanced_hybrid_model_v2"  # Dictionnaire pour les sorties nommées
    )
