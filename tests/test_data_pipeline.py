...
def test_run_pipeline_feature_validation_success(mock_apply_feature_pipeline, base_mock_df, tmp_path):
    ...
    with patch(f"{PIPELINE_MODULE_PATH}.load_raw_data", return_value=base_mock_df), \
         patch(f"{PIPELINE_MODULE_PATH}.clean_data", side_effect=lambda df: df), \
         patch("utils.llm_integration.LLMIntegration", autospec=True) as MockLLM, \
         patch(f"{PIPELINE_MODULE_PATH}.pd.DataFrame.to_parquet") as mock_to_parquet:
...
def test_run_pipeline_feature_validation_failure(mock_apply_feature_pipeline, base_mock_df, tmp_path):
    ...
    with patch(f"{PIPELINE_MODULE_PATH}.load_raw_data", return_value=base_mock_df), \
         patch(f"{PIPELINE_MODULE_PATH}.clean_data", side_effect=lambda df: df), \
         patch(f"{PIPELINE_MODULE_PATH}.build_labels", side_effect=lambda df: df), \
         patch("utils.llm_integration.LLMIntegration"), \
         patch(f"{PIPELINE_MODULE_PATH}.pd.DataFrame.to_parquet"):
