...
DEFAULT_LABEL_COLUMNS = training_params.get('label_columns', ['signal', 'volatility_quantiles', 'market_regime', 'sl_tp'])
...
def test_model_initialization(compiled_model):
    ...
    expected_outputs = {
        ...
        "conditional_output": (None, max(DEFAULT_OPTIONS_OUTPUT_DIM, DEFAULT_FUTURES_OUTPUT_DIM, 2)) # La dim dépend de l'implémentation de ConditionalOutputLayer
    }
...
def test_invalid_input_shapes(compiled_model, sample_data):
    ...
    X_bad_tech_tensors = {k: tf.convert_to_tensor(v, dtype=(tf.int64 if k == 'instrument_input' else tf.float32)) for k, v in X_bad_tech.items()}
    ...
    X_bad_llm_tensors = {k: tf.convert_to_tensor(v, dtype=(tf.int64 if k == 'instrument_input' else tf.float32)) for k, v in X_bad_llm.items()}
    with pytest.raises(ValueError):
    ...
    X_bad_mcp_tensors = {k: tf.convert_to_tensor(v, dtype=(tf.int64 if k == 'instrument_input' else tf.float32)) for k, v in X_bad_mcp.items()}
    with pytest.raises(ValueError):
    ...
    X_bad_inst['instrument_input'] = np.random.randint(0, DEFAULT_INSTRUMENT_VOCAB_SIZE, size=(NUM_TEST_SAMPLES, 2)).astype(np.int64)
    X_bad_inst_tensors = {k: tf.convert_to_tensor(v, dtype=(tf.int64 if k == 'instrument_input' else tf.float32)) for k, v in X_bad_inst.items()}
    with pytest.raises(ValueError):
...
def test_save_load_weights(compiled_model, sample_data, tmp_path):
    ...
    for key in original_pred:
        np.testing.assert_allclose(original_pred[key], new_pred[key], atol=1e-6,
                                   err_msg=f"Mismatch in output '{key}' after loading weights")
